...
import { draftText, emptyDraft, parseResume } from "@/lib/resume-builder";
...
export default function ResumeReviewer({ initialRole, initialMode = "review" }: { initialRole?: string; initialMode?: "review" | "build" }) {
  const [mode, setMode] = useState(initialMode);
  const [draft, setDraft] = useState(() => emptyDraft(careerById(initialRole || "")?.id || ""));
  ...
  function clear() { setDraft(emptyDraft(role)); generation.current++; setBusy(false); setText(""); setJd(""); setFileName(""); setReport(null); setSuggestions([]); setError(""); if (fileRef.current) fileRef.current.value = ""; }
  function download() {
    if (!report) return;
  ...
  function openBuilder(importText = false) {
    if (importText && text.trim()) {
      if (draftText(draft) && draftText(draft) !== text && !window.confirm("Replace your editor draft with this résumé text? Save your draft first if you want to keep it.")) return;
      if (draftText(draft) !== text) setDraft(parseResume(text, role));
    }
    setMode("build"); window.scrollTo({ top: 0, behavior: "instant" });
  }
  function checkDraft() {
    const content = draftText(draft); setText(content); setFileName(""); setRole(draft.targetRole); setError(""); setSuggestions(detectRoles(content));
    const target = careerById(draft.targetRole);
    setReport(target ? reviewResume(content, target, experience, jd) : null);
    if (!target) setError("Choose your target role to check this draft.");
    setMode("review"); window.scrollTo({ top: 0, behavior: "instant" });
  }
  if (mode === "build") return <ResumeBuilder draft={draft} onChange={setDraft} onCheck={checkDraft} onBack={() => { setMode("review"); window.scrollTo({ top: 0, behavior: "instant" }); }}/>;
  return <main className="career-hub"><div className="gc-container"><CareerNav active="resume"/><header className="ch-tool-hero"><span className="gc-kicker"><FileSearch size={15}/> RÉSUMÉ REVIEW</span><h1>Make your experience<br/><em>easy to see.</em></h1><p>Upload your résumé, choose your target role and get practical improvements, skill-gap prompts and relevant global certification options.</p><div className="ch-privacy"><LockKeyhole size={17}/><span>Your file is processed in this browser. The reviewer doesn’t upload or save your résumé.</span></div></header>
    <div className="rb-mode-nav"><span className="rb-mode rb-mode-active"><FileSearch size={18}/> Check résumé</span><button className="rb-mode" onClick={() => openBuilder()}>Create or edit résumé <ArrowRight size={18}/></button></div><div className="rb-review-entry"><div><strong>A clearer résumé, ready to download.</strong><p>Create from scratch or turn your uploaded résumé into an editable document.</p></div><button className="gc-button gc-button-light" disabled={busy} onClick={() => openBuilder(!!text.trim())}>{text.trim() ? "Edit & improve this résumé" : "Create my résumé"} <ArrowRight size={17}/></button></div>
    <div className="ch-review-layout"><section className="ch-input-panel" aria-label="Résumé input"><span className="gc-kicker">01 / YOUR EXPERIENCE & TARGET</span><h2>Let’s find your next step.</h2><div className="ch-upload"><Upload size={30}/><strong>{busy ? "Reading your document…" : fileName || "Choose your résumé"}</strong><p>PDF, DOCX or TXT · Up to 5 MB<br/>Text PDFs only; up to 20 pages</p><input ref={fileRef} id="resume-file" aria-label="Choose résumé file" type="file" accept=".pdf,.docx,.txt" disabled={busy} onChange={e => void onFile(e.target.files?.[0])}/></div><p className="ch-small">Check the extracted text below, especially columns and bullet points. Scanned PDFs need OCR or pasted text.</p>
      ...
      <section className="ch-review-results" ref={resultRef} tabIndex={-1} aria-label="Résumé review results" aria-live="polite">{!report ? <div className="ch-review-placeholder"><div className="ch-document-art" aria-hidden="true"><FileSearch size={55}/><i/><i/><i/><span><Check size={17}/> Your next step</span></div><span className="gc-kicker">02 / YOUR PERSONALISED REVIEW</span><h2>More clarity.<br/>A stronger application.</h2><p>Your review will appear here with:</p><ul><li><Check size={17}/> Skills mentioned and areas to make clearer</li><li><Check size={17}/> Practical résumé improvements</li><li><Check size={17}/> Certification options for your target role</li><li><Check size={17}/> A project and interview preparation path</li></ul></div> : <>
        ...
        <div className="ch-report-summary"><strong>{report.matched}</strong><div><h3>Skills mentioned · {report.skills.length - report.matched} to review</h3><p>Role skill checklist, not a résumé score. Text mentions do not verify ability or predict hiring.</p></div></div>
        ...
        <div className="ch-actions"><button className="gc-button gc-button-light" onClick={() => openBuilder(true)}>Edit & improve this résumé <ArrowRight size={17}/></button><button className="gc-button gc-button-outline" onClick={download}>Download review <Download size={17}/></button><Link className="gc-button gc-button-outline" href={`/careers/jobs?role=${report.path.id}`}>Explore jobs <ArrowUpRight size={17}/></Link></div>
