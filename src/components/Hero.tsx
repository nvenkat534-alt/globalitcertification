...
const Banner1 = () => {
  const certCards = [
    ...
    { name: "PMI PMP", subtitle: "Certified", img: "https://upload.wikimedia.org/wikipedia/commons/4/4b/Project_Management_Institute_Logo.svg" },
    { name: "ISACA", subtitle: "Certified", img: "https://upload.wikimedia.org/wikipedia/commons/0/0b/ISACA_logo.svg" },
  ...
  const marqueeLogos = [
    ...
    { name: "Google Cloud", img: "https://upload.wikimedia.org/wikipedia/commons/5/51/Google_Cloud_logo.svg" },
    { name: "Cisco", img: "https://upload.wikimedia.org/wikipedia/commons/0/08/Cisco_logo_blue_2016.svg" }
  ];

  return (
    <div className="w-full h-full flex flex-col justify-center gap-6 pt-6 pb-0 px-4 sm:px-6 lg:px-12 relative z-10 max-w-7xl mx-auto">
      <div className="flex flex-col lg:flex-row items-center justify-between gap-7 mt-0 lg:mt-0">
        
        {/* Left Column */}
        <div className="w-full lg:w-1/2 flex flex-col items-start text-left relative z-20">
          <div className="inline-block px-4 py-1.5 border border-brand-blue/30 rounded-full text-[10px] font-bold text-slate-300 mb-4 tracking-widest uppercase bg-brand-blue/20">
            THE GLOBAL STANDARD IN IT CERTIFICATIONS
          </div>
          
          <h1 className="text-[2.8rem] sm:text-[3.6rem] md:text-[5rem] font-bold mb-4 leading-[0.96] font-heading text-white tracking-tight">
            Get Certified.<br />
            In Your <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#5b6cff] to-[#5b6cff]">First Attempt.</span>
          </h1>
          
          <div className="flex items-center space-x-2 text-base text-slate-300 mb-5 font-medium">
          ...
          <div className="grid grid-cols-2 gap-3 mb-5 w-full max-w-lg">
            <div className="flex items-center space-x-2 text-sm">
              <i className="fas fa-globe text-brand-sky text-lg w-5"></i>
              <div className="flex flex-col">
                <span className="font-bold text-white text-xs">Globally Recognised</span>
                <span className="text-[10px] text-slate-400">Top Vendor Certifications</span>
              </div>
            </div>
            <div className="flex items-center space-x-2 text-sm">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3">
            <a href="/certifications" className="bg-[#5b6cff] text-white px-7 py-3 rounded-xl font-heading font-semibold text-base hover:bg-[#5b6cff] transition flex items-center justify-center w-full sm:w-auto">
              Explore Certifications <i className="fas fa-arrow-right ml-2"></i>
            </a>
            <a href="#" className="bg-transparent text-white border border-white/20 px-7 py-3 rounded-xl font-heading font-semibold text-base hover:bg-white/5 transition flex items-center justify-center w-full sm:w-auto">
              <i className="fas fa-play mr-2"></i> Watch How It Works
            </a>
          </div>
        </div>

        {/* Right Column - Cards */}
        <div className="w-full lg:w-1/2 relative z-20 flex justify-center lg:justify-end">
          <div className="grid grid-cols-2 gap-3 max-w-[460px] w-full">
            {certCards.map((card, i) => (
              <div key={i} className="group relative rounded-xl overflow-hidden bg-[#0A1128]/80 border border-brand-blue/30 backdrop-blur-md p-4 flex flex-col items-center justify-center aspect-[5/3] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_0_20px_rgba(91,108,255,0.35)] cursor-pointer hover:border-brand-sky/50">
      ...
      {/* Marquee Section */}
      <div className="w-full flex flex-col items-center relative z-20 pt-0 pb-0">
        <p className="text-center text-slate-300 font-sans text-[9px] sm:text-[10px] font-bold tracking-[0.16em] uppercase mb-2">
          Trusted by professionals from top companies
        </p>

        <div className="w-full overflow-hidden bg-transparent">
          <div className="animate-marquee flex w-max items-center gap-12 sm:gap-16 md:gap-20 whitespace-nowrap py-1">
            {[...marqueeLogos, ...marqueeLogos, ...marqueeLogos].map((company, index) => (
              <div key={`${company.name}-${index}`} className="flex items-center justify-center shrink-0 opacity-80">
                <img
                  src={company.img}
                  alt={company.name}
                  className="h-6 sm:h-7 md:h-8 w-auto object-contain brightness-0 invert opacity-60 transition-opacity duration-300 hover:opacity-100"
                  onError={(e) => {
                    e.currentTarget.style.display = 'none';
                    e.currentTarget.nextElementSibling?.classList.remove('hidden');
                  }}
    ...
    <div className="w-full h-full flex flex-col justify-center pt-32 pb-12 overflow-hidden relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full mb-4">
        <div className="flex flex-col items-center justify-center w-full mt-4 lg:mt-0 max-w-4xl mx-auto text-center">
          ...
          <div className="inline-block px-4 py-1.5 bg-brand-blue/20 border border-brand-blue/40 rounded-full text-[10px] font-bold text-brand-sky mb-4 font-heading tracking-widest uppercase shadow-[0_0_15px_rgba(91,108,255,0.3)]">
            The Global Standard in IT Certifications
          </div>
