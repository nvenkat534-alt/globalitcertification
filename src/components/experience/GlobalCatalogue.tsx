...
export default function GlobalCatalogue({items}: {items: Certification[]}) {
  ...
  return <main className="global-catalogue catalogue-hub">
    ...
    <section className="catalogue-workspace gc-container" id="all-certifications" aria-label="Certification catalogue">
      ...
      {showProviders && <div className="catalogue-provider-grid" id="catalogue-provider-directory">{orderedProviders.map(p => <button key={p.id} onClick={() => chooseProvider(p.id)} aria-pressed={provider === p.id}><ProviderMark provider={p.id}/><span>{counts.get(p.id) || 0} {counts.get(p.id) === 1 ? "certification" : "certifications"} <ArrowRight size={14}/></span></button>)}</div>}
