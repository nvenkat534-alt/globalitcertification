...
export default function Navbar() {
  ...
  return <>
    ...
    <header className="gc-nav"><div className="gc-container gc-nav-inner">
      ...
      <div className="gc-nav-actions"><MotionSettings/><a className="gc-nav-whatsapp" href={whatsappUrl("Hi Global Certs IT! I would like details about a global certification.")} target="_blank" rel="noopener noreferrer"><WhatsAppIcon size={18}/><span>WhatsApp us</span><ArrowUpRight size={14}/></a><button ref={toggle} className="gc-mobile-toggle" aria-label={open ? "Close navigation" : "Open navigation"} aria-expanded={open} aria-controls="gc-mobile-navigation" onClick={()=>setOpen(!open)}>{open ? <X size={24}/> : <Menu size={24}/>}</button></div>
