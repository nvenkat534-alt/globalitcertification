    ...
    <section className="py-24 bg-white relative z-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold font-heading text-brand-dark mb-4">Featured Programs</h2>
          <p className="text-lg text-slate-600 max-w-2xl mx-auto">Explore our most popular personalized training tracks designed to get you certified and hired.</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {programs.map((prog, i) => (
            ...
            >
              ...
              <h3 className="text-2xl font-bold text-brand-dark mb-3 font-heading">{prog.title}</h3>
              <p className="text-slate-600 mb-6 flex-grow">{prog.desc}</p>
              
              <div className="space-y-3 mb-8">
                <div className="flex justify-between border-b border-slate-100 pb-2">
                  <span className="text-slate-500 text-sm font-medium">Who it's for</span>
                  <span className="text-brand-dark text-sm font-semibold text-right max-w-[60%]">{prog.for}</span>
                </div>
                <div className="flex justify-between border-b border-slate-100 pb-2">
                  <span className="text-slate-500 text-sm font-medium">Mode</span>
                  <span className="text-brand-dark text-sm font-semibold text-right">{prog.mode}</span>
                </div>
                <div className="flex justify-between border-b border-slate-100 pb-2">
                  <span className="text-slate-500 text-sm font-medium">Key Skills</span>
                  <span className="text-brand-dark text-sm font-semibold text-right max-w-[60%] truncate">{prog.skills}</span>
                </div>
                <div className="flex justify-between pb-2">
                  <span className="text-slate-500 text-sm font-medium">Careers</span>
                  <span className="text-brand-dark text-sm font-semibold text-right max-w-[60%] truncate">{prog.careers}</span>
                </div>
              </div>

              <Link href={prog.link} className="w-full py-4 rounded-xl font-bold font-heading text-center transition-all bg-slate-50 text-brand-dark hover:bg-brand-blue hover:text-white flex items-center justify-center space-x-2 border border-slate-200 hover:border-brand-blue group">
