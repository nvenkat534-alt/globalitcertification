...
const { normaliseJob, plainText, isTechJob } = await import("../src/lib/job-types.ts");
...
assert.equal(isTechJob(job), true);
assert.equal(isTechJob({ title: "Corporate Law Specialist", tags: ["legal"] }), false);
assert.equal(isTechJob({ title: "Senior SAP SD Consultant", tags: [] }), true);
